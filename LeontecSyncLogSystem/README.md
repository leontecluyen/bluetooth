...
```
   Android (client)                LogManagement.exe  (1 tiến trình)
...
# db log_management, user root, password rỗng). App tự tạo DB + schema qua EF migrations lúc khởi động.
...
(mặc định `localhost:3306`, `log_management`, `root`, rỗng).
