...
- **DB = MySQL ngoài (cài/chạy riêng).** MariaDB nhúng đã **gỡ bỏ** — app không đóng gói/khởi động DB
  ...
  thiếu file thì tạo mặc định `localhost:3306`, `leontec_sync`, `root`, rỗng) → dựng connection string.
- Provider DB = **MySQL/MariaDB qua Pomelo** (`UseMySql(...).UseSnakeCaseNamingConvention()`). Phiên bản
  server **ghim cứng** (`new MySqlServerVersion(10.4.0)`) thay cho `ServerVersion.AutoDetect` — AutoDetect
  mở kết nối ngay lúc cấu hình và sẽ ném lỗi nếu MySQL đang tắt. Schema tạo/nâng cấp lúc khởi động bằng
  **EF Core migrations** (`db.Database.Migrate()`). **DB tắt lúc khởi động KHÔNG làm app crash**: lỗi
  được log (Error) rồi app vẫn mở, dashboard hiện "MySQL: disconnected"; migrations/roster nạp ở lần chạy
  sau khi MySQL đã lên. Sau khi migrate OK thì **nạp roster thiết bị** và `ServiceStatus.SeedFromPersisted(...)`.
...
DB = **MySQL/MariaDB ngoài** (cài/chạy riêng; kết nối theo `mysql.xml`). Schema qua EF Core migrations
(`Data/Migrations/`), tạo lúc khởi động. Data do server MySQL bên ngoài quản lý.
